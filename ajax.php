...
$fields = [
    ...
    'application'=> isset($data['applicationNumber']) ? htmlspecialchars(strip_tags($data['applicationNumber'])) : null,
    'typeOfLoan'=> isset($data['typeOfLoan']) ? htmlspecialchars(strip_tags($data['typeOfLoan'])) : null,
];
...
if (isset($fields['u_id'])) {
    $u_id = $fields['u_id'];
    $checkStmt = $conn->prepare("SELECT COUNT(*) FROM bajajfinservleads_test WHERE u_id = ? AND mobile = ?");
    ...
    if ($count > 0) {
        ...
        $updateStmt = $conn->prepare("UPDATE bajajfinservleads_test SET $updateFields WHERE u_id = ?");
